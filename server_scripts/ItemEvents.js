...
// --- 粘液棒：生成粘液块平台 ---
ItemEvents.rightClicked("rainbow:slime_rod", event => {
...
// --- 末影戒指：打开末影箱 ---
/*ItemEvents.rightClicked("rainbow:enderchest", event => {
...
// --- 饕餮之锅：食物收集 ---
ItemEvents.rightClicked("rainbow:eldritch_pan", event => {
    let { player, item, level } = event;
    if (level.isClientSide()) return;
...
/*
// --- 饕餮剑：剑吞噬 ---
ItemEvents.rightClicked("rainbow:eldritch_sword", event => {
    let { player, item, level } = event;
    if (level.isClientSide()) return;
...
ItemEvents.rightClicked('rainbow:terasword', event => {
    ...
    if (nbt.getInt("power")) {
        // 消耗能量
        nbt.putInt("power", nbt.getInt("power") - 1);
...
/*
// --- 乐谱：播放音乐 ---
ItemEvents.rightClicked("rainbow:musical_score", event => {
    let { item, level } = event;
    if (level.isClientSide()) return;
    if (!item.nbt) return;
...
ItemEvents.rightClicked("rainbow:mini_ender_chest", event => {
    let { player, level } = event;
    if (level.isClientSide()) return;
    player.openInventoryGUI(player.enderChestInventory, Component.translatable("container.enderchest"));
});
